# TESTES — Feature de Compressão de Imagens

Criar `tests/Feature/CompressaoTest.php`:
...
```bash
# Só os testes de compressão
php artisan test --filter=CompressaoTest

# Todos os testes do projeto
php artisan test
```

22 testes cobrindo: compressão JPG/PNG, redimensionamento, thumbnail quadrado, suporte a extensões, upload prévia vs completo, campo no banco, galeria usa thumbnail, download entrega original, deleção limpa thumbnail.

Se algum falhar, corrija o código da feature (não o teste).
